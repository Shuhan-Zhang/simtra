...
impl PumsRecord {
    ...
    /// PUMS `SEX` is a two-category administrative field (1 = male, 2 = female);
    /// it records recorded sex, not gender identity. Labels stay coarse to match.
    ...
    pub fn nativity_label(&self) -> &'static str {
        if self.foreign_born() {
            "foreign_born"
        } else {
            "us_born"
        }
    }
    /// Mirrors [`Self::employed`]: everyone not in the employed ESR codes —
    /// including the under-16 N/A code — falls into `not_employed`.
    ...
    pub fn citizenship_label(&self) -> &'static str {
        if self.is_citizen() {
            "citizen"
        } else {
            "noncitizen"
