...
impl MemoryClient {
    ...
    pub async fn record_reactions(
        &self, workspace: &str, pop: &Population, event_id: &str, reactions: &[Reaction],
    ) -> Result<()> {
        let population_key = population_key_in(workspace, pop);
        ...
        self.run(&[(
            "MATCH (e:Event {id: $event})-[:HAPPENED_IN]->(:City {key: $event_city}) \
             MERGE (c:City {key: $city}) ON CREATE SET c.slug = $slug, c.workspace = 'public' \
            ...
            json!({"event": event_id, "rows": rows, "pop": population_key,
                "city": city_key(PUBLIC_WORKSPACE, &pop.profile.slug), "slug": pop.profile.slug, "event_city": city_key(workspace, &pop.profile.slug), "seed": pop.seed as i64, "n": pop.n as i64, "now": now_iso()}),
        )]).await?;
        Ok(())
    }
