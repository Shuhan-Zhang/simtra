...
/// Group option distributions by key. Output follows first-seen key order; callers
/// can apply a domain-specific semantic order without any HashMap iteration leaking.
/// Returns (key, normalized shares, group weight, n).
pub fn option_breakdown<K: Eq + std::hash::Hash + Clone>(
    rows: &[(K, WeightedAnswer)],
    n_options: usize,
) -> Vec<(K, Vec<f64>, f64, usize)> {
    use std::collections::HashMap;
    let mut index: HashMap<K, usize> = HashMap::new();
    let mut groups: Vec<(K, Vec<WeightedAnswer>)> = Vec::new();
    for (key, answer) in rows {
        ...
        groups[idx].1.push(answer.clone());
    }
    groups
        .into_iter()
        .map(|(key, answers)| {
            let weight = answers.iter().map(|a| a.weight).sum();
            let n = answers.len();
            (key, weighted_distribution(&answers, n_options), weight, n)
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn option_breakdown_is_weighted_normalized_and_stable() {
        let rows = vec![
            (
                "first",
                WeightedAnswer {
        ...
        let groups = option_breakdown(&rows, 2);
        assert_eq!(
            groups.iter().map(|g| g.0).collect::<Vec<_>>(),
            vec!["first", "second"]
        );
        assert!((groups[0].1[0] - 0.9).abs() < 1e-12);
        assert!((groups[0].1.iter().sum::<f64>() - 1.0).abs() < 1e-12);
        assert_eq!((groups[0].2, groups[0].3), (10.0, 2));
    }

    #[test]
    fn option_breakdown_zero_weight_uses_uniform_fallback() {
        let rows = vec![
            (
                "zero",
                WeightedAnswer {
        ...
        let groups = option_breakdown(&rows, 2);
        assert_eq!(groups[0].1, vec![0.5, 0.5]);
        assert_eq!((groups[0].2, groups[0].3), (0.0, 2));
    }
