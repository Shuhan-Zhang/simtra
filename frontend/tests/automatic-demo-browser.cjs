...
(async () => {
  ...
  try {
    ...
    await page.evaluate(async()=>{
      ...
      app.state.news=[{headline:'Timeline verification headline',summary:'Fixture news beside saved experiments.',date:'2026-09-18'}];
      (await import('./src/feedpanel.js?v=18')).showCityTimeline();
    });
    assert.equal(await page.locator('.fp-thread .fp-post-news').count(),1);
